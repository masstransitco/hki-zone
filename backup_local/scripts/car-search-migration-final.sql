...
-- Final version without generated column dependencies
...
-- 3. Add computed columns for normalized make/model (no dependencies)
ALTER TABLE articles 
...
-- 4. Add search_text as a regular column (not generated) to avoid dependency issues
ALTER TABLE articles
ADD COLUMN IF NOT EXISTS search_text tsvector;

-- Create a function to update search_text
  ...
  IF NEW.category = 'cars' THEN
    NEW.search_text := 
      setweight(
        to_tsvector('simple', 
          COALESCE(
            COALESCE((NEW.specs->>'車廠')::text, (NEW.specs->>'make')::text, split_part(trim(NEW.title), ' ', 1)),
            ''
          )
        ), 'A'
      ) ||
      setweight(
        to_tsvector('simple', 
          COALESCE(
            COALESCE((NEW.specs->>'型號')::text, (NEW.specs->>'model')::text, trim(substring(NEW.title from position(' ' in NEW.title) + 1))),
            ''
          )
        ), 'B'
      ) ||
...
DROP TRIGGER IF EXISTS update_car_search_text_trigger ON articles;
CREATE TRIGGER update_car_search_text_trigger
  BEFORE INSERT OR UPDATE ON articles
  FOR EACH ROW
  EXECUTE FUNCTION update_car_search_text();
...
SET search_text = 
  setweight(
    to_tsvector('simple', 
      COALESCE(
        COALESCE((specs->>'車廠')::text, (specs->>'make')::text, split_part(trim(title), ' ', 1)),
        ''
      )
    ), 'A'
  ) ||
  setweight(
    to_tsvector('simple', 
      COALESCE(
        COALESCE((specs->>'型號')::text, (specs->>'model')::text, trim(substring(title from position(' ' in title) + 1))),
        ''
      )
    ), 'B'
  ) ||
...
COMMENT ON COLUMN articles.search_text IS 'Full-text search vector for make/model/title with weights, updated by trigger';
