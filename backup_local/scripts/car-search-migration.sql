...
-- Based on the optimization guide for Postgres + Supabase

-- 1. Enable fuzzy matching extension
CREATE EXTENSION IF NOT EXISTS pg_trgm;

-- 2. Add computed columns for normalized make/model
...
-- 3. Add search_text tsvector column for full-text search
ALTER TABLE articles
...
-- 5. Create search function with input validation
...
BEGIN
  ...
  SELECT 
    ...
    a.make,
    a.model,
    (a.specs->>'售價')::text as price,
    (a.specs->>'年份')::text as year,
    a.image_url,
    a.images,
    a.url,
    a.created_at,
...
-- 6. Create autocomplete function for live suggestions
...
-- 7. Create function to get filter options
...
BEGIN
  ...
  SELECT 
    ...
    jsonb_build_object(
      'under_100k', (SELECT COUNT(*) FROM articles a WHERE a.category = 'cars' AND (regexp_replace((a.specs->>'售價'), '[^0-9]', '', 'g'))::int < 100000),
      'range_100_300k', (SELECT COUNT(*) FROM articles a WHERE a.category = 'cars' AND (regexp_replace((a.specs->>'售價'), '[^0-9]', '', 'g'))::int BETWEEN 100000 AND 299999),
      'range_300_500k', (SELECT COUNT(*) FROM articles a WHERE a.category = 'cars' AND (regexp_replace((a.specs->>'售價'), '[^0-9]', '', 'g'))::int BETWEEN 300000 AND 499999),
      'range_500k_1m', (SELECT COUNT(*) FROM articles a WHERE a.category = 'cars' AND (regexp_replace((a.specs->>'售價'), '[^0-9]', '', 'g'))::int BETWEEN 500000 AND 999999),
      'over_1m', (SELECT COUNT(*) FROM articles a WHERE a.category = 'cars' AND (regexp_replace((a.specs->>'售價'), '[^0-9]', '', 'g'))::int >= 1000000)
    ) as price_ranges;
END;
$$ LANGUAGE plpgsql;
