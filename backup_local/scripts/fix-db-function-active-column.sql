-- Fix the database function to remove reference to non-existent active column
...
BEGIN
  ...
  WITH feed_names AS (
    ...
    WHERE f.active = true  -- Only check active on feeds, not incidents
  )
  SELECT 
    i.id,
    i.feed_id,
    fn.display_name as feed_name,
    fn.base_slug as feed_slug,
  ...
  WHERE 
    ...
    -- Remove the AND i.active = true line since this column doesn't exist
