...
async function extractArticleContent(url) {
  ...
  try {
    ...
    // Remove ONCC-specific navigation and UI elements
    $('.bknSocialShare, .social-share, .share-btn').remove();
    $('[class*="tweet"], [class*="facebook"], [class*="social"]').remove();
    $('.bknTVBox, .video-player, .media-player').remove();
    $('time, .time, .date, .timestamp').remove();
    $('[class*="navigation"], [class*="breadcrumb"]').remove();
    $('.ads, .advertisement, [class*="ad-"]').remove();
    ...
    for (const sel of selectors) {
      const element = $(sel);
      if (element.length > 0) {
        // Extract paragraphs to maintain formatting
        const paragraphs = [];
    ...
    if (body) {
      body = body
        ...
        .join(' ')  // Join sentences with single space
        // Split into logical paragraphs (every 2-3 sentences or at natural breaks)
        .replace(/([。！？\.!?])\s*([A-Z\u4e00-\u9fff])/g, '$1\n\n$2'); // Add paragraph breaks
    }

    // Fallback: extract from all paragraphs if main content extraction failed
    if (!body || body.length < 80) {
      const paragraphs = $('p').map((_, p) => $(p).text().trim())
                               .get()
                               .filter(x => x.length > 10 && 
                                          !x.includes('Tweet') && 
                                          !x.includes('東網電視') &&
                                          !/\d{4}年\d{2}月\d{2}日/.test(x));
      if (paragraphs.length > 0) {
        body = paragraphs.join('\n\n');
      }
    }
