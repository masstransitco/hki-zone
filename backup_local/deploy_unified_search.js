...
async function deployUnifiedSearch() {
  ...
  try {
    ...
    console.log('Creating unified search function...');
    
    // Deploy the unified search function
    const unifiedSearchSQL = `
...
`;
    ...
    await client.query(unifiedSearchSQL);
    console.log('✓ Unified search function created');
    
    // Update the main search function to use the unified version
...
`;
    ...
    await client.query(updateMainFunction);
    console.log('✓ Main search function updated to use unified version');
    
    // Grant permissions
    await client.query('GRANT EXECUTE ON FUNCTION search_car_listings_unified TO anon, authenticated;');
    await client.query('GRANT EXECUTE ON FUNCTION search_car_listings TO anon, authenticated;');
    console.log('✓ Permissions granted');
    
    console.log('Successfully deployed unified search function!');
    
  } catch (error) {
    console.error('Error deploying unified search:', error);
  } finally {
    await client.end();
  }
}
