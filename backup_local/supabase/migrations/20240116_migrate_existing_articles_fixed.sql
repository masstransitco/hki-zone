...
-- FIXED: Properly handle key_points conversion from text[] to jsonb
...
SELECT 
    ...
    summary,
    lede,
    -- Convert text[] to jsonb for key_points
    CASE 
        WHEN key_points IS NOT NULL AND array_length(key_points, 1) > 0
        THEN to_jsonb(key_points)
        ELSE NULL
    END as key_points,
    why_it_matters,
    image_url,
