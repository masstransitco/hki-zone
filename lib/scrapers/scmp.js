...
import { processContent, assessContentQuality } from '../content-processor.js';
...
const RSS_FEEDS = [
  ...
  ["https://www.scmp.com/rss/6/feed",  "tech"],
  ["https://www.scmp.com/rss/7/feed",  "lifestyle"],
  // Note: feed 8 returns 404, removed
];
...
async function fetchRSSListings() {
  ...
  return uniqueItems.slice(0, 100); // Increased limit to 100 articles
}
    ...
    try {
      ...
      const article = Array.isArray(json) 
        ? json.find(item => item && item["@type"] && (
            ...
            (Array.isArray(item["@type"]) && item["@type"].some(type => type === "NewsArticle" || type === "Article"))
          ))
        : (json["@type"] && (
            json["@type"] === "NewsArticle" || 
            json["@type"] === "Article" ||
            (typeof json["@type"] === "string" && json["@type"].includes("NewsArticle")) ||
            (Array.isArray(json["@type"]) && json["@type"].some(type => type === "NewsArticle" || type === "Article"))
          )) ? json : null;
...
function extractBodyFallback(html) {
  ...
  for (const regex of selectors) {
    ...
    if (match && match[1] && match[1].length > 300) {
      ...
      const cleanParagraphs = paragraphs
        ...
        .filter(text => text.length > 20 && !text.match(/^[\s\)\(\;]+$/))
        .filter(text => 
  ...
  // Enhanced final fallback - extract all paragraphs with better filtering
  const allParagraphs = html.match(/<p[^>]*>([\s\S]*?)<\/p>/gi) || [];
  const cleanAllParagraphs = allParagraphs
    .map(p => p.replace(/<[^>]*>/g, '').trim())
    .filter(text => 
      text.length > 30 && 
      !text.match(/^[\s\)\(\;]+$/) && 
      ...
      !text.match(/^[\d\s\-\.\,]+$/) && // Remove lines with only numbers/punctuation
      text.split(' ').length > 5 // Ensure meaningful content
    );
...
async function fetchArticle(rssItem) {
  try {
    ...
    const processedBody = processContent(cleanedBody, 'scmp');
    const quality = assessContentQuality({ body: processedBody, headline, coverImg: image, date, url: canonical });

    // Log quality issues for monitoring
    if (quality.quality === 'poor') {
      console.warn(`[${SOURCE_NAME} quality] Poor content quality for ${canonical}: ${quality.issues.join(', ')}`);
    }
    ...
    return { 
      ...
      _quality: quality // Internal quality metadata
    };
  } catch (e) {
...
export async function scrapeSCMP() {
  ...
  // Process articles with improved rate limiting
  const concurrentLimit = 8; // Increased from 4 to 8
  for (let i = 0; i < rssItems.length; i += concurrentLimit) {
    ...
    const promises = batch.map(async (rssItem) => {
      await sleep(150); // Reduced from 250ms to 150ms
  ...
  // Quality statistics
  const qualityStats = results.reduce((stats, article) => {
    const quality = article._quality?.quality || 'unknown';
    stats[quality] = (stats[quality] || 0) + 1;
    return stats;
  }, {});

  console.log(`[${SOURCE_NAME}] Completed scrape: ${results.length} articles`);
  console.log(`[${SOURCE_NAME}] Quality distribution:`, qualityStats);
  
  // Remove internal quality metadata before returning
  return results.map(({ _quality, ...article }) => article);
}