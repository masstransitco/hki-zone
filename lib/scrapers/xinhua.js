...
async function extractArticleContent(url) {
  try {
    ...
    if (!imageUrl || imageUrl.includes('sharelogo')) {
      ...
      if (imgMatch && !imgMatch[1].includes('logo') && !imgMatch[1].includes('icon')) {
        ...
        if (!imgSrc.startsWith('http')) {
          // Get base path from article URL (e.g., https://english.news.cn/northamerica/20260102/xxx/)
    ...
    // Also check for Xinhua-specific image patterns in the HTML
    if (!imageUrl || imageUrl.includes('sharelogo')) {
      const xinhuaImgMatch = cleanHtml.match(/src="([^"]*zxcode_[^"]+\.(?:jpg|jpeg|png|gif))"/i)
      if (xinhuaImgMatch) {
        let imgSrc = xinhuaImgMatch[1]
        if (!imgSrc.startsWith('http')) {
          const urlParts = url.match(/(https?:\/\/[^/]+\/.*\/\d{8}\/[a-f0-9]+)\//)
          if (urlParts) {
            imgSrc = `${urlParts[1]}/${imgSrc.replace(/^\.\//, '')}`
