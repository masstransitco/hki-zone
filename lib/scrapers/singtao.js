...
const XML_FEEDS = [
  "https://std.stheadline.com/realtime/rss.xml",
  "https://std.stheadline.com/sc/realtime/rss.xml",
  "https://std.stheadline.com/rss/news.xml",
]

// URL Cleaning Function
function cleanSingTaoURL(url) {
  // Extract just the article ID from SingTao URLs
  const match = url.match(/https:\/\/std\.stheadline\.com\/realtime\/article\/(\d+)/)
  if (match) {
    const articleId = match[1]
    return `https://std.stheadline.com/realtime/article/${articleId}/`
  }
  return url
}
        ...
        for (const itemMatch of itemMatches.slice(0, 15)) {
          ...
          if (titleMatch && linkMatch) {
            ...
            if (title.length > 5 && url) {
              articles.push({
                title,
                url: url.startsWith("http") ? url : `https://std.stheadline.com${url}`,
                description,
                pubDate,
              })
            }
...
async function fromHtml() {
  try {
    ...
    // Try multiple pages
    const pages = [
      "https://std.stheadline.com/realtime",
      "https://std.stheadline.com/",
      "https://std.stheadline.com/latest",
    ]
    ...
    for (const pageUrl of pages) {
      try {
        ...
        const articleLinkRegex = /<a[^>]*href="([^"]*\/(?:realtime\/)?article[^"]*)"[^>]*>([^<]+)<\/a>/gi
        let match
  ...
  return [
    {
      ...
      url: "https://std.stheadline.com/realtime/article/mock1",
      description: "香港經濟在多項政策支持下顯示復甦跡象",
      pubDate: new Date().toISOString(),
    },
    {
      title: "本港科技發展迎來新機遇，創新企業獲政府資助",
      url: "https://std.stheadline.com/realtime/article/mock2",
      description: "政府宣布新的科技創新支援計劃",
      pubDate: new Date(Date.now() - 3600000).toISOString(),
    },
    {
      title: "教育改革新方向，加強STEM課程培養未來人才",
      url: "https://std.stheadline.com/realtime/article/mock3",
...
async function extractArticleContentSimple(url) {
  ...
  try {
    ...
    // Strategy 1: Look for article image
    ...
    if (!imageUrl) {
      ...
      if (contentImageMatch) {
        ...
        if (src.includes("stheadline.com") || src.includes("static") || src.includes("images")) {
          imageUrl = src
        }
      }
    }
    ...
    if (imageUrl) {
      if (imageUrl.startsWith("//")) {
        ...
        imageUrl = "https://std.stheadline.com" + imageUrl
      }
...
async function scrapeSingTaoWithContent() {
  ...
  // Check if RSS already has content
  const hasRSSContent = headlines.some((h) => h.description && h.description.length > 50)

  if (hasRSSContent) {
    console.log("SingTao: Using RSS content (no individual page requests needed)")
  ...
  // Try only 3 articles max to avoid triggering blocking
  const articlesToTry = Math.min(uniqueHeadlines.length, 3)
