  ...
  try {
    ...
    for (const selector of contentSelectors) {
      ...
      if (contentMatch && contentMatch[1].trim().length > 200) {
        // Extract paragraphs from content using improved regex
        const paragraphMatches = contentMatch[1].match(/<p[^>]*>([\s\S]*?)<\/p>/gi)
        if (paragraphMatches) {
    ...
    if (!content || content.length < 100) {
      // Use improved regex that handles nested tags better
      ...
      if (allParagraphs) {
        content = allParagraphs
          ...
          .filter((text) => text.length > 20 && 
                           ...
                           !text.includes('立即下載') &&
                           !text.includes('更多文章') &&
                           !/^【.*】$/.test(text.trim())) // Filter out tag-only lines like 【港聞】
          .join("\n\n")
      }
    }
    ...
    if (content && content.length > 100) {
      ...
      if (!trimmed.match(/[。！？\.]$/)) {
        ...
        if (lastSentenceEnd > 0) {
          content = trimmed.substring(0, lastSentenceEnd + 1);
        }
      }
