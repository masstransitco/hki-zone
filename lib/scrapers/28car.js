...
// Function to get chromium based on environment
async function getChromium() {
  if (!isProduction) {
    return null; // Use local Chrome in development
  }
  
  try {
    // Dynamic import for production to avoid webpack issues
    const chromiumModule = await import('@sparticuz/chromium');
    return chromiumModule.default;
  } catch (err) {
    console.warn('⚠️ @sparticuz/chromium not available, falling back to local Chrome');
    return null;
  }
}
...
async function scrape28CarWithContent() {
  ...
  // Get chromium instance for serverless environments
