...
function extractBody(html) {
  ...
  const cleanHtml = html
    ...
    .replace(/<header[^>]*class="[^"]*article__head[^"]*"[^>]*>[\s\S]*?<\/header>/gi, '')
    .replace(/<footer[^>]*>[\s\S]*?<\/footer>/gi, '')
    .replace(/<!--[\s\S]*?-->/g, '') // Remove HTML comments
    .replace(/<aside[^>]*class="[^"]*newsflash[^"]*"[^>]*>[\s\S]*?<\/aside>/gi, ''); // Remove related articles

  // Strategy 1: Look for itemprop="articleBody" and extract everything until related articles
  let content = '';
  const articleBodyMatch = cleanHtml.match(/<div[^>]*itemprop="articleBody"[^>]*>([\s\S]*?)(?=<aside|<div[^>]*class="[^"]*article__foot|$)/i);
  if (articleBodyMatch) {
    content = articleBodyMatch[1];
  }

  // Strategy 2: Try article__body class
  if (!content) {
    const articleClassMatch = cleanHtml.match(/<div[^>]*class="[^"]*article__body[^"]*"[^>]*>([\s\S]*?)(?=<aside|<div[^>]*class="[^"]*article__foot|$)/i);
    if (articleClassMatch) {
      content = articleClassMatch[1];
    }
  }
  ...
  // Remove ad boxes from content
  content = content
    .replace(/<div[^>]*class="[^"]*adbox[^"]*"[^>]*>[\s\S]*?<\/div>/gi, '')
    .replace(/<figure[^>]*>[\s\S]*?<\/figure>/gi, '') // Remove figures (images with captions)
    .replace(/<div[^>]*id="[^"]*ad[^"]*"[^>]*>[\s\S]*?<\/div>/gi, ''); // Remove ad divs

  // AM730 content structure: <div><p><span><span>...text...</span></span></p></div>
  // Extract text from all div>p structures
  const texts = [];

  // Find all divs containing paragraphs with content
  const divParagraphs = content.match(/<div>[\s\S]*?<p[^>]*>[\s\S]*?<\/p>[\s\S]*?<\/div>/gi) || [];
  for (const divP of divParagraphs) {
    ...
    if (text && text.length > 10 && isValidContent(text)) {
      texts.push(text);
    }
  }

  // Also try standalone paragraphs (not in divs)
  if (texts.length < 2) {
    ...
    for (const p of paragraphs) {
      ...
      if (text && text.length > 10 && isValidContent(text) && !texts.includes(text)) {
        texts.push(text);
      }
    }
  }
...
function isValidContent(text) {
  ...
  const invalidPatterns = [
    /^(返回|分享：|熱門搜尋|支持AM730|Facebook|Instagram|Youtube|搜尋|登出|我的收藏|積分及獎賞|活動列表|帳戶概覽|相關新聞|更多新聞|訂閱|通知|廣告)$/i,
    /ADVERTISEMENT/i,          // Filter any line containing ADVERTISEMENT
    /熱門搜尋:/,
    /window\._taboola/,
    /_taboola\.push/,
    /element reset style/,
    ...
    /^分享$/,
    /^相關$/,
    /激送.*門票/,              // Filter promotional content
    /送出.*獎品/,
    /抽獎活動/,
  ];
  ...
  // Must contain at least some Chinese characters for AM730
  const chineseChars = text.match(/[\u4e00-\u9fff]/g);
  if (!chineseChars || chineseChars.length < 5) return false;
