...
const NewsfeedIcon = ({ className, isActive }: { className?: string; isActive?: boolean }) => (
  <div className="relative flex items-center justify-center">
    {isActive && (
      <div className="absolute inset-0 bg-nav-active scale-75 rounded-md" />
    )}
    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor" className={cn(className, "relative z-10", isActive ? "text-surface-1" : "")}>
...
const BookmarkIcon = ({ className, isActive }: { className?: string; isActive?: boolean }) => (
  <div className="relative flex items-center justify-center">
    {isActive && (
      <TurnedInIcon className={cn(className, "absolute text-nav-active")} />
    )}
    <TurnedInNotIcon className={cn(className, "relative z-10", isActive ? "text-surface-1" : "text-current")} />
  </div>
)
...
const MarketplaceIcon = ({ className, isActive }: { className?: string; isActive?: boolean }) => (
  <div className="relative flex items-center justify-center">
    {isActive && (
      <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor" className={cn(className, "absolute text-nav-active")}>
    ...
    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor" className={cn(className, "relative z-10", isActive ? "text-surface-1" : "text-current")}>
...
export default function FooterNav() {
  ...
  return (
    <>
      <nav 
        className="fixed bottom-0 left-0 right-0 z-30 bg-surface-1/95 backdrop-blur-sm border-t border-card-border/60 pb-safe"
        style={{
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  // Enhanced semantic color handling
                  isActive 
                    ? "text-nav-active" 
                    : "text-nav-inactive hover:text-text-2",
